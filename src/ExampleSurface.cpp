...
void ExampleSurface::clear(void)
{
}
...
inline void ExampleSurface::getRayIntersection(double pa[3],double pb[3], vector<pair<VERTEX_TYPE,VERTEX_TYPE*>> &intersections, bool computeNormals, int thread_id)
{
	...
	if (hasIntersection)
	{
		...
		{
			double n1[3], n2[3];
			getNormalToSphere(intPoint1,center,radius,n1);
			getNormalToSphere(intPoint2,center,radius,n2);

			float *normal1 = allocateVector<float>(3);
			float *normal2 = allocateVector<float>(3);
			...
			intersections.push_back(pair<float,float*>(t1,normal1));
			intersections.push_back(pair<float,float*>(t2,normal2));
		}
		else
		{
			intersections.push_back(pair<float,float*>(t1,(float*)NULL));
			intersections.push_back(pair<float,float*>(t2,(float*)NULL));
		}
		#endif
...
inline void ExampleSurface::printRayIntersection(double pa[3], double pb[3])
{
	;
}
#endif
...
void ExampleSurface::printSummary(void)
{
	cout << endl << INFO << "Example surface has a radius of " << radius;
}


void ExampleSurface::preProcessPanel(void)
{
...
void ExampleSurface::postRayCasting(void)
{
	// if we used an acceleration data structure we could clean it here
}


bool ExampleSurface::preBoundaryProjection(void)
{
	// if we used an acceleration data structure we could build it here
	return true;
}
...
int ExampleSurface::getNumPatches (void)
{
...
bool ExampleSurface::isPatchBasedRayTracingSupported (void)
{
	return false;
}


bool ExampleSurface::build(void)
{
