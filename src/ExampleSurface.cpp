...
void ExampleSurface::init(ConfigurationOP cf) {
  // we have the configuration file; so we can query the radius parameter (or
  // our custom parameters) the second argument is the default value if the
  // radius keyword is not found radius = cf->read<double>(
  // "Example_Surface_Parameter", DEFAULT_RADIUS);
  radius = cf->radius;
}
...
inline void ExampleSurface::getRayIntersection(
  ...
  // intersections contains as first value the parameter t of the ray
  // pa+t*(pb-pa) and the second is the normal vector
  double dir[3];
  dir[0] = pb[0] - pa[0];
  dir[1] = pb[1] - pa[1];
  dir[2] = pb[2] - pa[2];

  // we know the maximal number of intersections in this case; if we don't know
  // that we can make a guess
  intersections.reserve(2);

  // in real scenario you should preliminarly identify a superset of patches
  // that could intersect the ray and then test only them. In this trival case
  // we have only one patch, the sphere

  // these are two t parameters of the ray. For one sphere we can have up to 2
  // intersections
  double t1, t2;
  ...
  // the variable 'panel' is controlled by the mother class. You should not
  // touch it it identifies the panel from which we are casting rays
...
bool ExampleSurface::getProjection(double p[3], double *proj1, double *proj2,
  ...
  // only the intersection routine is needed.

  // in a real scenario you should identify the nearest patch and then project
  // to it; you don't have to wonder if p must be projected or not. p has been
  // already found as a boundary grid point, that is a point in the nearby of
  // the surface, so for sure we have to project it. The only caveat is that for
  // numerical reasons you could not be able to project it. In this last case
  // the safest thing to do is to set as the projection the (grid) point itself;
...
void ExampleSurface::printSummary() {
  logging::log<logging::level::info>("Example surface has a radius of {}",
                                     radius);
}
...
void ExampleSurface::preProcessPanel() {
  ...
  // the acceleration data structure dependes on the panel from which we are
  // tracing rays.
}
...
bool ExampleSurface::build() {
  ...
  if (maxDist < radius) {
    logging::log<logging::level::warn>(
        "Max distance is lower than radius. Setting radius = max distance");
    radius = maxDist;
  }
  return true;
}
