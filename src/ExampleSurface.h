...
introduce a new surface in NanoShaper. This class builds a sphere centered on
the baricenter of input points whose radius is specified by the user. This
surface is ray-casting based and it is a trivial Surface definition. On a real
scenario high performance is needed; to get performance the Developer can use
the same strategies used in other surfaces such as in classes ConnollySurface
and SkinSurface. The acceleration strategies are 2D/3D grids that allow fast
retrieval of points or in general of simplices and patches. To add a new surface
it is sufficient that you define a class withi this style (see registration
class) and then it will be automatically loaded; you don't have to change any
make file or cmake file, just write the source and put it the src folder.
  ...
  void setRadius(double r) {
    if (r <= 0) {
      logging::log<logging::level::warn>(
          "Cannot set a radius <0. Setting the default value");
...
you have to follow the naming convention that follows in the class. The
following variable declaration, (please look at it) works perfectly and builds
automatically this class but Swig seems to have problem with it so we write it
explicitly*/
