...
bool SkinSurface::buildSkinCGAL() {
  ...
  for (int i = 0; i < delphi->numAtoms; i++) {
    ...
    delphi->atoms[i]->pos[1] =
        delphi->atoms[i]->pos[1] + randDisplacement * (randnum() - 0.5);
    delphi->atoms[i]->pos[2] =
        delphi->atoms[i]->pos[2] + randDisplacement * (randnum() - 0.5);

    x = (float)delphi->atoms[i]->pos[0];
    y = (float)delphi->atoms[i]->pos[1];
    z = (float)delphi->atoms[i]->pos[2];
    r = (float)delphi->atoms[i]->radius;

    l.emplace_back(Weighted_point(Point(x, y, z), (r * r) / s), i);
    max_x = max(max_x, x + r);
    max_y = max(max_y, y + r);
    max_z = max(max_z, z + r);
  ...
  // atoms
  l.emplace_back(Weighted_point(Point(min_x, mid_y, mid_z), -1),
                 delphi->numAtoms);
  ...
  /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  /////////////////////////// Compute Voronoi points and reduced Delaunay solids
  ///(Skin) ///////////////////////////////////
  /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  for (; fcit != rT.finite_cells_end(); fcit++, currentCell++) {
    ...
    plane3points(mc->reduced[2], mc->reduced[1], mc->reduced[0], mc->planes[0],
                 false);
    plane3points(mc->reduced[3], mc->reduced[2], mc->reduced[0], mc->planes[1],
                 false);
    plane3points(mc->reduced[0], mc->reduced[1], mc->reduced[3], mc->planes[3],
                 false);
    plane3points(mc->reduced[3], mc->reduced[1], mc->reduced[2], mc->planes[2],
                 false);

    plane3points(mc->points[2], mc->points[1], mc->points[0], mc->planes_or[0],
                 false);
    plane3points(mc->points[3], mc->points[2], mc->points[0], mc->planes_or[1],
                 false);
    plane3points(mc->points[0], mc->points[1], mc->points[3], mc->planes_or[3],
                 false);
    plane3points(mc->points[3], mc->points[1], mc->points[2], mc->planes_or[2],
                 false);
    ...
    // order of points in producing the planes leverages the combinatoric
    // properties CGAL internal data structures. In a few words, using points on
    ...
    QQ[1] = 1;
    QQ[2] = 1;
    QQ[3] = -radius2 * (1 - s) + vor[0] * vor[0] + vor[1] * vor[1] +
            vor[2] * vor[2];
    QQ[4] = 0;
    QQ[7] = 0;
    QQ[9] = -vor[0];
  ...
  /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  /////////////////////////// Compute Voronoi (VDW) or reduced Voronoi solids
  ///(Skin) //////////////////////////////////////
  /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  for (; fvit != rT.finite_vertices_end(); fvit++) {
    ...
    QQ[1] = 1;
    QQ[2] = 1;
    QQ[3] = -s * (fvit->point().weight()) +
            fvit->point().x() * fvit->point().x() +
            fvit->point().y() * fvit->point().y() +
            fvit->point().z() * fvit->point().z();
    QQ[4] = 0;
    QQ[7] = 0;
    QQ[9] = -fvit->point().x();
    QQ[5] = 0;
  ...
  for (; fei != rT.finite_edges_end(); fei++) {
    ...
    for (int i = 0; i < num; i++) {
      ...
      // remember that the first point is up, the second down, the third up and
      // so on
      if (i == 2) {
    ...
    // decide orientation based on the position of the focus of the computed
    // planes
    sign = DOT(mc->upper, inner) + mc->upper[3];
    if (sign > 0)
      ASSIGN4(mc->upper, -mc->upper)
  ...
  for (; ffi != rT.finite_facets_end(); ffi++) {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      // store the weight of the del point that will be used to compute the
      // focus
      if (ind == 0)
    ...
    plane3points(mc->mc_points[0], mc->mc_points[2], mc->mc_points[4],
                 mc->upper);
    // orient
    sign = DOT(mc->upper, inner) + mc->upper[3];
    if (sign > 0)
    ...
    plane3points(mc->mc_points[1], mc->mc_points[3], mc->mc_points[5],
                 mc->lower);
    // orient
    sign = DOT(mc->lower, inner) + mc->lower[3];
    if (sign > 0)
...
void SkinSurface::saveSkinPatch(ofstream &of, MixedCell *mc, int index,
  ...
  if (mc->surface_type == DELAUNAY_EDGE_CELL) {
    ...
    snprintf(clipplanes, sizeof(clipplanes), "Clipping_Mesh%d", index);
    of << "\n#declare " << clipplanes << "=\n mesh{";
    ...
    for (fit = poly.facets_begin(); fit != poly.facets_end(); fit++) {
      ...
      for (h = fit->facet_begin(); g < 3; h++) {
        of << "<" << h->vertex()->point().x() << "," << h->vertex()->point().y()
           << "," << h->vertex()->point().z() << ">";
        if (g < 2)
          of << ",";
        g++;
    ...
    t1[0] =
        (ec->mc_points[0][0] + ec->mc_points[2][0] + ec->mc_points[4][0]) / 3.;
    t1[1] =
        (ec->mc_points[0][1] + ec->mc_points[2][1] + ec->mc_points[4][1]) / 3.;
    t1[2] =
        (ec->mc_points[0][2] + ec->mc_points[2][2] + ec->mc_points[4][2]) / 3.;

    t2[0] =
        (ec->mc_points[1][0] + ec->mc_points[3][0] + ec->mc_points[5][0]) / 3.;
    t2[1] =
        (ec->mc_points[1][1] + ec->mc_points[3][1] + ec->mc_points[5][1]) / 3.;
    t2[2] =
        (ec->mc_points[1][2] + ec->mc_points[3][2] + ec->mc_points[5][2]) / 3.;
    ...
    // this is formally correct but Pov-Ray seems not to be able to manage this
    // code for more than very small molecules
    /*
    // add clipping planes
    of << "\n\n #declare " << clipplanes << " =" ;
    ...
    */
    ...
    auto p = std::get<0>(la[pc->id]);
    snprintf(buff2, sizeof(buff2), "\n\n sphere { \n <%f,%f,%f>,%f", p.x(),
             p.y(), p.z(), sqrt(p.weight() * s));
    of << buff2;
    of << "\n pigment{color Green}}";
    ...
    t1[0] =
        (fc->mc_points[0][0] + fc->mc_points[2][0] + fc->mc_points[4][0]) / 3.;
    t1[1] =
        (fc->mc_points[0][1] + fc->mc_points[2][1] + fc->mc_points[4][1]) / 3.;
    t1[2] =
        (fc->mc_points[0][2] + fc->mc_points[2][2] + fc->mc_points[4][2]) / 3.;

    t2[0] =
        (fc->mc_points[1][0] + fc->mc_points[3][0] + fc->mc_points[5][0]) / 3.;
    t2[1] =
        (fc->mc_points[1][1] + fc->mc_points[3][1] + fc->mc_points[5][1]) / 3.;
    t2[2] =
        (fc->mc_points[1][2] + fc->mc_points[3][2] + fc->mc_points[5][2]) / 3.;
    ...
    */
    ...
    of << "\n// ----------------------------------------------------//";
    of << "\n// atoms " << tc->ids[0] << "," << tc->ids[1] << "," << tc->ids[2]
       << "," << tc->ids[3];
    ...
    for (unsigned int i = 0; i < 4; i++) {
      ...
      pp[1] = tc->reduced[i][1];
      pp[2] = tc->reduced[i][2];
      local.push_back(Point(pp[0] + 1e-3 * (randnum() - 0.5),
                            pp[1] + 1e-3 * (randnum() - 0.5),
                            pp[2] + 1e-3 * (randnum() - 0.5)));
    }
    ...
    of << "\n inside_vector<" << tc->vor[0] << "," << tc->vor[1] << ","
       << tc->vor[2] << ">";
    of << "}";
    ...
    // code for more than very small molecules
    /*
    snprintf(clipplanes, sizeof(clipplanes), "Clipping_Planes%d",index);
    of << "\n\n #declare " << clipplanes << " =" ;
    of << "\n intersection {";
    for (int l=0;l<4;l++)
    {
...
void SkinSurface::preProcessPanel() {
  ...
  gridMixedCellMap2D =
      allocateVector<int>(last_rows_ind * last_cols_ind * MAX_MIXEDCELLS_2D);
...
inline bool SkinSurface::rayQuadricIntersection(double *orig, double *dir,
  ...
  // double
  // q44=Q[3][3],q33=Q[2][2],q22=Q[1][1],q11=Q[0][0],q34=Q[2][3],q24=Q[1][3],q23=Q[1][2],q14=Q[0][3],q13=Q[0][2],q12=Q[0][1];
  double q44 = Q[3], q33 = Q[2], q22 = Q[1], q11 = Q[0], q34 = Q[6], q24 = Q[8],
         q23 = Q[5], q14 = Q[9], q13 = Q[7], q12 = Q[4];

  a = cache[0] * q44 + cache[1] * q34 + cache[2] * q33 + cache[3] * q24 +
      cache[4] * q23 + cache[5] * q22 + cache[6] * q14 + cache[7] * q13 +
      cache[8] * q12 + cache[9] * q11;
  ...
  b = cache[20] * q44 + cache[21] * q34 + cache[22] * q33 + cache[23] * q24 +
      cache[24] * q23 + cache[25] * q22 + cache[26] * q14 + cache[27] * q13 +
      cache[28] * q12 + cache[29] * q11;

  // for (int i=0;i<4;i++)
  //	for (int j=0;j<4;j++)
  //		c+=orig[i]*Q[i][j]*orig[j];

  c = cache[10] * q44 + cache[11] * q34 + cache[12] * q33 + cache[13] * q24 +
      cache[14] * q23 + cache[15] * q22 + cache[16] * q14 + cache[17] * q13 +
      cache[18] * q12 + cache[19] * q11;
...
SkinSurface::getRayIntersection(double pa[3], double pb[3],
  ...
  // this cache exploits the particular structure of skin ray-tracing in order
  // to improve speed. all intersections are with quadrics so the ray invariant
  // part is precomputed once and for all.
  double cache[30];
  ...
  if (panel == 0) {
    ...
    for (int iter = 0; iter < numcells; iter++) {
      ...
      bool ff = rayQuadricIntersection(pat, dirt, mixedComplex[it]->quadric,
                                       &t1, &t2, thdID, cache);
      ...
      // feasibility test: check the intersection is inside the mixed complex
      // cell
      ...
      if (testFeasibility) {
        ...
        if (computeNormals) {
          ...
          normal[1] = 2 * (Q[1] * intPoint[1] + Q[4] * intPoint[0] +
                           Q[5] * intPoint[2] + Q[8]);
          normal[2] = 2 * (Q[2] * intPoint[2] + Q[7] * intPoint[0] +
                           Q[5] * intPoint[1] + Q[6]);
...
void SkinSurface::projectToQuadric(double *y, double *Q, int type, double *proj,
  ...
  poly[6] =
      16 *
      (a11 * a22 * a33 - a122 * a33 - a11 * a232 + 2 * a12 * a13 * a23 -
       a132 * a22) *
      (4 * a11 * a22 * a33 * c - 4 * a122 * a33 * c - 4 * a11 * a232 * c +
       8 * a12 * a13 * a23 * c - 4 * a132 * a22 * c - b02 * a22 * a33 +
       2 * b[0] * b[1] * a12 * a33 - b12 * a11 * a33 + b02 * a232 -
       2 * b[0] * b[1] * a13 * a23 - 2 * b[0] * b[2] * a12 * a23 +
       2 * b[1] * b[2] * a11 * a23 + 2 * b[0] * b[2] * a13 * a22 -
       b22 * a11 * a22 + b12 * a132 - 2 * b[1] * b[2] * a12 * a13 + b22 * a122);

  // t^5 coefficient
  poly[5] =
      16 * (a22 * a33 + a11 * a33 - a232 + a11 * a22 - a132 - a122) *
      (4 * a11 * a22 * a33 * c - 4 * a122 * a33 * c - 4 * a11 * a232 * c +
       8 * a12 * a13 * a23 * c - 4 * a132 * a22 * c - b02 * a22 * a33 +
       2 * b[0] * b[1] * a12 * a33 - b12 * a11 * a33 + b02 * a232 -
       2 * b[0] * b[1] * a13 * a23 - 2 * b[0] * b[2] * a12 * a23 +
  ...
  // t^4 coefficient
  poly[4] = +(
      ((16 * a222 + 64 * a11 * a22 - 32 * a122 + 16 * a112) * a332 +
       ((-32 * a22 - 64 * a11) * a232 + 64 * a12 * a13 * a23 + 64 * a11 * a222 +
        (-64 * a132 - 64 * a122 + 64 * a112) * a22 - 32 * a11 * a132 -
        64 * a11 * a122) *
           a33 +
       16 * a234 +
       (-64 * a11 * a22 + 32 * a132 + 32 * a122 - 32 * a112) * a232 +
       (64 * a12 * a13 * a22 + 64 * a11 * a12 * a13) * a23 +
       (16 * a112 - 32 * a132) * a222 +
       (-64 * a11 * a132 - 32 * a11 * a122) * a22 + 16 * a134 +
       32 * a122 * a132 + 16 * a124) *
          c +
      ((16 * y02 * a11 + 16 * b[0] * y[0]) * a222 +
       (-16 * y02 * a122 +
        (-32 * y[0] * y[1] * a11 - 16 * b[0] * y[1] - 16 * y[0] * b[1]) * a12 +
        16 * y12 * a112 - 4 * b12 - 16 * b02) *
           a22 +
       32 * y[0] * y[1] * a123 +
       (-16 * y12 * a11 + 16 * b[1] * y[1] + 16 * b[0] * y[0]) * a122 +
       ((-16 * b[0] * y[1] - 16 * y[0] * b[1]) * a11 + 24 * b[0] * b[1]) * a12 +
       16 * b[1] * y[1] * a112 + (-16 * b12 - 4 * b02) * a11) *
          a332 +
      (((-32 * y02 * a11 - 32 * b[0] * y[0]) * a22 + 16 * y02 * a122 +
        (32 * y[0] * y[1] * a11 + 16 * b[0] * y[1] + 16 * y[0] * b[1]) * a12 -
        16 * y12 * a112 + 4 * b12 + 16 * b02) *
           a232 +
       (((32 * y02 * a12 + 32 * y[0] * y[1] * a11 + 16 * b[0] * y[1] +
          16 * y[0] * b[1]) *
             a13 +
         (32 * y[0] * y[2] * a11 + 16 * b[0] * y[2] + 16 * y[0] * b[2]) * a12 -
         32 * y[1] * y[2] * a112 + 8 * b[1] * b[2]) *
            a22 +
        (-96 * y[0] * y[1] * a122 +
         (32 * y12 * a11 - 32 * b[1] * y[1] - 32 * b[0] * y[0]) * a12 +
         (16 * b[0] * y[1] + 16 * y[0] * b[1]) * a11 - 24 * b[0] * b[1]) *
            a13 -
        32 * y[0] * y[2] * a123 +
        (32 * y[1] * y[2] * a11 - 16 * b[1] * y[2] - 16 * y[1] * b[2]) * a122 +
        ((16 * b[0] * y[2] + 16 * y[0] * b[2]) * a11 - 24 * b[0] * b[2]) * a12 +
        (-16 * b[1] * y[2] - 16 * y[1] * b[2]) * a112 +
        32 * b[1] * b[2] * a11) *
           a23 +
       (-16 * y02 * a132 +
        (-32 * y[0] * y[2] * a11 - 16 * b[0] * y[2] - 16 * y[0] * b[2]) * a13 +
        16 * y22 * a112 - 4 * b22 - 16 * b02) *
           a222 +
       ((32 * y[0] * y[1] * a12 - 32 * y12 * a11) * a132 +
        (32 * y[0] * y[2] * a122 +
         (32 * y[1] * y[2] * a11 + 16 * b[1] * y[2] + 16 * y[1] * b[2]) * a12 +
         32 * b[0] * b[2]) *
            a13 -
        32 * y22 * a11 * a122 + 32 * b[0] * b[1] * a12 +
        (-16 * b22 - 16 * b12 - 16 * b02) * a11) *
           a22 +
       (16 * y12 * a122 + (16 * b[0] * y[1] + 16 * y[0] * b[1]) * a12 -
        32 * b[1] * y[1] * a11 + 16 * b12 + 4 * b02) *
           a132 +
       (-32 * y[1] * y[2] * a123 +
        (-16 * b[0] * y[2] - 16 * y[0] * b[2]) * a122 +
        ((16 * b[1] * y[2] + 16 * y[1] * b[2]) * a11 - 24 * b[1] * b[2]) * a12 +
        8 * b[0] * b[2] * a11) *
           a13 +
       16 * y22 * a124 + 8 * b22 * a122 + 32 * b[0] * b[1] * a11 * a12 +
       (-4 * b22 - 16 * b12) * a112) *
          a33 +
      (16 * y02 * a11 + 16 * b[0] * y[0]) * a234 +
      ((-32 * y02 * a12 - 32 * y[0] * y[1] * a11 - 16 * b[0] * y[1] -
        16 * y[0] * b[1]) *
           a13 +
       (-32 * y[0] * y[2] * a11 - 16 * b[0] * y[2] - 16 * y[0] * b[2]) * a12 +
      ...
      ((16 * y02 * a132 +
        ...
        16 * y22 * a112 + 4 * b22 + 16 * b02) *
           a22 +
       (64 * y[0] * y[1] * a12 + 16 * y12 * a11 + 16 * b[1] * y[1] +
        16 * b[0] * y[0]) *
           a132 +
       (64 * y[0] * y[2] * a122 +
        (-96 * y[1] * y[2] * a11 + 16 * b[1] * y[2] + 16 * y[1] * b[2]) * a12 +
        (-16 * b[0] * y[2] - 16 * y[0] * b[2]) * a11 - 8 * b[0] * b[2]) *
           a13 +
       (16 * y22 * a11 + 16 * b[2] * y[2] + 16 * b[0] * y[0]) * a122 +
       ((-16 * b[0] * y[1] - 16 * y[0] * b[1]) * a11 - 8 * b[0] * b[1]) * a12 +
       (16 * b[2] * y[2] + 16 * b[1] * y[1]) * a112 + 8 * b02 * a11) *
          a232 +
      ((-32 * y[0] * y[1] * a133 +
        (-96 * y[0] * y[2] * a12 + 32 * y[1] * y[2] * a11 - 16 * b[1] * y[2] -
         16 * y[1] * b[2]) *
            a132 +
        ((32 * y22 * a11 - 32 * b[2] * y[2] - 32 * b[0] * y[0]) * a12 +
         (16 * b[0] * y[1] + 16 * y[0] * b[1]) * a11 - 24 * b[0] * b[1]) *
            a13 +
        ((16 * b[0] * y[2] + 16 * y[0] * b[2]) * a11 - 24 * b[0] * b[2]) * a12 +
        (-16 * b[1] * y[2] - 16 * y[1] * b[2]) * a112 +
       ...
       (64 * y[1] * y[2] * a122 + (16 * b[0] * y[2] + 16 * y[0] * b[2]) * a12 +
        (16 * b[1] * y[2] + 16 * y[1] * b[2]) * a11 - 8 * b[1] * b[2]) *
           a132 +
       (-32 * y22 * a123 + (16 * b[0] * y[1] + 16 * y[0] * b[1]) * a122 +
        ((-32 * b[2] * y[2] - 32 * b[1] * y[1]) * a11 + 8 * b22 + 8 * b12 +
         8 * b02) *
            a12 -
        24 * b[0] * b[1] * a11) *
           a13 +
       (-16 * b[0] * y[2] - 16 * y[0] * b[2]) * a123 +
       ((16 * b[1] * y[2] + 16 * y[1] * b[2]) * a11 - 8 * b[1] * b[2]) * a122 -
       24 * b[0] * b[2] * a11 * a12 + 24 * b[1] * b[2] * a112) *
          a23 +
      (32 * y[0] * y[2] * a133 +
       (-16 * y22 * a11 + 16 * b[2] * y[2] + 16 * b[0] * y[0]) * a132 +
       ((-16 * b[0] * y[2] - 16 * y[0] * b[2]) * a11 + 24 * b[0] * b[2]) * a13 +
       16 * b[2] * y[2] * a112 + (-16 * b22 - 4 * b02) * a11) *
          a222 +
      (16 * y12 * a134 - 32 * y[1] * y[2] * a12 * a133 +
       (16 * y22 * a122 + (-16 * b[0] * y[1] - 16 * y[0] * b[1]) * a12 +
        8 * b12) *
           a132 +
       ((16 * b[0] * y[2] + 16 * y[0] * b[2]) * a122 +
        ((16 * b[1] * y[2] + 16 * y[1] * b[2]) * a11 - 24 * b[1] * b[2]) * a12 +
        32 * b[0] * b[2] * a11) *
           a13 +
       (-32 * b[2] * y[2] * a11 + 16 * b22 + 4 * b02) * a122 +
       8 * b[0] * b[1] * a11 * a12 + (-16 * b22 - 4 * b12) * a112) *
          a22 +
      16 * b[1] * y[1] * a134 +
      ((-16 * b[1] * y[2] - 16 * y[1] * b[2]) * a12 - 8 * b[0] * b[2]) * a133 +
      ((16 * b[2] * y[2] + 16 * b[1] * y[1]) * a122 - 8 * b[0] * b[1] * a12 +
       (4 * b22 + 16 * b12) * a11) *
          a132 +
      ((-16 * b[1] * y[2] - 16 * y[1] * b[2]) * a123 - 8 * b[0] * b[2] * a122 -
       24 * b[1] * b[2] * a11 * a12) *
          a13 +
      16 * b[2] * y[2] * a124 - 8 * b[0] * b[1] * a123 +
      (16 * b22 + 4 * b12) * a11 * a122);

  // t^3 coefficient
  poly[3] = +(
      ((16 * a22 + 16 * a11) * a332 +
       (-16 * a232 + 16 * a222 + 64 * a11 * a22 - 16 * a132 - 32 * a122 +
        16 * a112) *
           a33 +
       (-16 * a22 - 32 * a11) * a232 + 32 * a12 * a13 * a23 + 16 * a11 * a222 +
       (-32 * a132 - 16 * a122 + 16 * a112) * a22 - 16 * a11 * a132 -
       16 * a11 * a122) *
          c +
      (((16 * y12 + 16 * y02) * a11 + 16 * b[0] * y[0]) * a22 +
       (-16 * y12 - 16 * y02) * a122 +
       (-16 * b[0] * y[1] - 16 * y[0] * b[1]) * a12 + 16 * b[1] * y[1] * a11 -
       4 * b12 - 4 * b02) *
          a332 +
      (((-16 * y12 - 16 * y02) * a11 - 16 * b[0] * y[0]) * a232 +
       (-32 * y[1] * y[2] * a11 * a22 +
        ((32 * y12 + 32 * y02) * a12 + 16 * b[0] * y[1] + 16 * y[0] * b[1]) *
            a13 +
        32 * y[1] * y[2] * a122 + (16 * b[0] * y[2] + 16 * y[0] * b[2]) * a12 +
        (-16 * b[1] * y[2] - 16 * y[1] * b[2]) * a11 + 8 * b[1] * b[2]) *
           a23 +
       ((16 * y22 + 16 * y02) * a11 + 16 * b[0] * y[0]) * a222 +
       ((-16 * y12 - 16 * y02) * a132 +
        (-32 * y[0] * y[2] * a11 - 16 * b[0] * y[2] - 16 * y[0] * b[2]) * a13 +
        (-16 * y22 - 16 * y02) * a122 +
        (-32 * y[0] * y[1] * a11 - 16 * b[0] * y[1] - 16 * y[0] * b[1]) * a12 +
        (16 * y22 + 16 * y12) * a112 - 4 * b22 - 4 * b12 - 16 * b02) *
           a22 -
       16 * b[1] * y[1] * a132 +
       (32 * y[0] * y[2] * a122 + (16 * b[1] * y[2] + 16 * y[1] * b[2]) * a12 +
        8 * b[0] * b[2]) *
           a13 +
       32 * y[0] * y[1] * a123 +
       ((-16 * y22 - 16 * y12) * a11 + 16 * b[1] * y[1] + 16 * b[0] * y[0]) *
           a122 +
       ((-16 * b[0] * y[1] - 16 * y[0] * b[1]) * a11 + 24 * b[0] * b[1]) * a12 +
       16 * b[1] * y[1] * a112 + (-4 * b22 - 16 * b12 - 4 * b02) * a11) *
          a33 +
      32 * y[1] * y[2] * a11 * a233 +
      (((-16 * y22 - 16 * y02) * a11 - 16 * b[0] * y[0]) * a22 +
       (32 * y[0] * y[2] * a11 - 64 * y[1] * y[2] * a12) * a13 +
       32 * y[0] * y[1] * a11 * a12 + (-16 * y22 - 16 * y12) * a112 +
       (16 * b[2] * y[2] + 16 * b[1] * y[1]) * a11 + 8 * b02) *
          a232 +
      ((32 * y[1] * y[2] * a132 +
        ((32 * y22 + 32 * y02) * a12 + 16 * b[0] * y[1] + 16 * y[0] * b[1]) *
            a13 +
        (16 * b[0] * y[2] + 16 * y[0] * b[2]) * a12 +
        (-16 * b[1] * y[2] - 16 * y[1] * b[2]) * a11 + 8 * b[1] * b[2]) *
           a22 -
       64 * y[0] * y[2] * a12 * a132 +
       (-64 * y[0] * y[1] * a122 +
        ((32 * y22 + 32 * y12) * a11 - 32 * b[2] * y[2] - 32 * b[1] * y[1] -
         32 * b[0] * y[0]) *
            a12 +
        (16 * b[0] * y[1] + 16 * y[0] * b[1]) * a11 - 16 * b[0] * b[1]) *
           a13 +
       ((16 * b[0] * y[2] + 16 * y[0] * b[2]) * a11 - 16 * b[0] * b[2]) * a12 +
       (-16 * b[1] * y[2] - 16 * y[1] * b[2]) * a112 + 24 * b[1] * b[2] * a11) *
          a23 +
      ((-16 * y22 - 16 * y02) * a132 +
       (-16 * b[0] * y[2] - 16 * y[0] * b[2]) * a13 + 16 * b[2] * y[2] * a11 -
       4 * b22 - 4 * b02) *
          a222 +
      (32 * y[0] * y[2] * a133 +
       (32 * y[0] * y[1] * a12 + (-16 * y22 - 16 * y12) * a11 +
        16 * b[2] * y[2] + 16 * b[0] * y[0]) *
           a132 +
       ((16 * b[1] * y[2] + 16 * y[1] * b[2]) * a12 +
        (-16 * b[0] * y[2] - 16 * y[0] * b[2]) * a11 + 24 * b[0] * b[2]) *
           a13 -
       16 * b[2] * y[2] * a122 + 8 * b[0] * b[1] * a12 +
       16 * b[2] * y[2] * a112 + (-16 * b22 - 4 * b12 - 4 * b02) * a11) *
          a22 +
      (8 * b12 - 16 * b[1] * y[1] * a11) * a132 +
      (((16 * b[1] * y[2] + 16 * y[1] * b[2]) * a11 - 16 * b[1] * b[2]) * a12 +
       8 * b[0] * b[2] * a11) *
          a13 +
      (8 * b22 - 16 * b[2] * y[2] * a11) * a122 + 8 * b[0] * b[1] * a11 * a12 +
      (-4 * b22 - 4 * b12) * a112);

  // t^2 coefficient
  poly[2] =
      +((4 * a332 + (16 * a22 + 16 * a11) * a33 - 8 * a232 + 4 * a222 +
         16 * a11 * a22 - 8 * a132 - 8 * a122 + 4 * a112) *
            c +
        (4 * y12 * a22 + 8 * y[0] * y[1] * a12 + 4 * y02 * a11 +
         4 * b[1] * y[1] + 4 * b[0] * y[0]) *
            a332 +
        (-4 * y12 * a232 +
         (-8 * y[1] * y[2] * a22 - 8 * y[0] * y[1] * a13 -
          8 * y[0] * y[2] * a12 - 4 * b[1] * y[2] - 4 * y[1] * b[2]) *
             a23 +
         4 * y22 * a222 +
         ((16 * y22 + 16 * y12 + 16 * y02) * a11 + 16 * b[0] * y[0]) * a22 -
         4 * y02 * a132 +
         (-8 * y[1] * y[2] * a12 - 8 * y[0] * y[2] * a11 - 4 * b[0] * y[2] -
          4 * y[0] * b[2]) *
             a13 +
         (-8 * y22 - 16 * y12 - 16 * y02) * a122 +
         (-16 * b[0] * y[1] - 16 * y[0] * b[1]) * a12 + 4 * y22 * a112 +
         16 * b[1] * y[1] * a11 - b22 - 4 * b12 - 4 * b02) *
            a33 +
        8 * y[1] * y[2] * a233 +
        (-4 * y22 * a22 + 8 * y[0] * y[2] * a13 + 8 * y[0] * y[1] * a12 +
         (-16 * y22 - 16 * y12 - 8 * y02) * a11 + 4 * b[2] * y[2] +
         4 * b[1] * y[1] - 8 * b[0] * y[0]) *
            a232 +
        ((-8 * y[0] * y[1] * a13 - 8 * y[0] * y[2] * a12 - 4 * b[1] * y[2] -
          4 * y[1] * b[2]) *
             a22 +
         8 * y[1] * y[2] * a132 +
         ((24 * y22 + 24 * y12 + 24 * y02) * a12 - 8 * y[0] * y[1] * a11 +
          12 * b[0] * y[1] + 12 * y[0] * b[1]) *
             a13 +
         8 * y[1] * y[2] * a122 +
         (-8 * y[0] * y[2] * a11 + 12 * b[0] * y[2] + 12 * y[0] * b[2]) * a12 +
         8 * y[1] * y[2] * a112 + (-16 * b[1] * y[2] - 16 * y[1] * b[2]) * a11 +
         6 * b[1] * b[2]) *
            a23 +
        (8 * y[0] * y[2] * a13 + 4 * y02 * a11 + 4 * b[2] * y[2] +
         4 * b[0] * y[0]) *
            a222 +
        ((-16 * y22 - 8 * y12 - 16 * y02) * a132 +
         (-8 * y[1] * y[2] * a12 - 16 * b[0] * y[2] - 16 * y[0] * b[2]) * a13 -
         4 * y02 * a122 +
         (-8 * y[0] * y[1] * a11 - 4 * b[0] * y[1] - 4 * y[0] * b[1]) * a12 +
         4 * y12 * a112 + 16 * b[2] * y[2] * a11 - 4 * b22 - b12 - 4 * b02) *
            a22 +
        8 * y[0] * y[2] * a133 +
        (8 * y[0] * y[1] * a12 - 4 * y22 * a11 + 4 * b[2] * y[2] -
         8 * b[1] * y[1] + 4 * b[0] * y[0]) *
            a132 +
        (8 * y[0] * y[2] * a122 +
         (-8 * y[1] * y[2] * a11 + 12 * b[1] * y[2] + 12 * y[1] * b[2]) * a12 +
         (-4 * b[0] * y[2] - 4 * y[0] * b[2]) * a11 + 6 * b[0] * b[2]) *
            a13 +
        8 * y[0] * y[1] * a123 +
        (-4 * y12 * a11 - 8 * b[2] * y[2] + 4 * b[1] * y[1] + 4 * b[0] * y[0]) *
            a122 +
        ((-4 * b[0] * y[1] - 4 * y[0] * b[1]) * a11 + 6 * b[0] * b[1]) * a12 +
        (4 * b[2] * y[2] + 4 * b[1] * y[1]) * a112 +
        (-4 * b22 - 4 * b12 - b02) * a11);

  // t coefficient
  poly[1] =
      ((4 * a33 + 4 * a22 + 4 * a11) * c +
       ((4 * y22 + 4 * y12) * a22 + 8 * y[0] * y[1] * a12 +
        (4 * y22 + 4 * y02) * a11 + 4 * b[1] * y[1] + 4 * b[0] * y[0]) *
           a33 +
       (-4 * y22 - 4 * y12) * a232 +
       (-8 * y[0] * y[1] * a13 - 8 * y[0] * y[2] * a12 + 8 * y[1] * y[2] * a11 -
        4 * b[1] * y[2] - 4 * y[1] * b[2]) *
           a23 +
       (8 * y[0] * y[2] * a13 + (4 * y12 + 4 * y02) * a11 + 4 * b[2] * y[2] +
        4 * b[0] * y[0]) *
           a22 +
       (-4 * y22 - 4 * y02) * a132 +
       (-8 * y[1] * y[2] * a12 - 4 * b[0] * y[2] - 4 * y[0] * b[2]) * a13 +
       (-4 * y12 - 4 * y02) * a122 +
       (-4 * b[0] * y[1] - 4 * y[0] * b[1]) * a12 +
       (4 * b[2] * y[2] + 4 * b[1] * y[1]) * a11 - b22 - b12 - b02);

  // t^0 coefficient
  poly[0] = +c + y22 * a33 + 2 * y[1] * y[2] * a23 + y12 * a22 +
            2 * y[0] * y[2] * a13 + 2 * y[0] * y[1] * a12 + y02 * a11 +
            b[2] * y[2] + b[1] * y[1] + b[0] * y[0];
  ...
  // skip non properly projected patch (typical case of distant patch of no
  // interest)
  if (num == 0) {
    // printf("\n %lf %lf %lf %lf %lf %lf
    // %lf",poly[6],poly[5],poly[4],poly[3],poly[2],poly[1],poly[0]);
    dist = INFINITY;
    // getchar();
    return;
  }
  ...
  for (int i = 0; i < num; i++) {
    ...
    double den =
        aa * ee * ii + 2 * dd * fg - gg * gg * ee - dd * dd * ii - aa * ff2;
    double num1 = jj * ee * ii + dd * ff * ll + kk * fg - gg * ee * ll -
                  dd * kk * ii - jj * ff2;
    double num2 = aa * kk * ii + jj * fg + gg * dd * ll - gg * gg * kk -
                  jj * dd * ii - aa * ff * ll;
    double num3 = aa * ee * ll + dd * kk * gg + jj * dd * ff - jj * ee * gg -
                  dd * dd * ll - aa * kk * ff;
