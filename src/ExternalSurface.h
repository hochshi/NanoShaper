...
public:
  ...
  /** Approximate the volume by summing the inner boxes of the grid plus the
  partial volumes of the boundary boxes.*/
...
public:
  ExternalSurfaceRegister() {
    SurfaceFactory::getInstance().register_instantiator("external",
                                                        createSurface);
  }
