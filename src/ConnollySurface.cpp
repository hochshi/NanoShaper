...
bool ConnollySurface::buildAuxiliaryGrid() {
  if (sesComplex.size() == 0) {
    logging::log<logging::level::warn>(
        "Cannot get surface with an empty SES complex!");
    return false;
  }
  ...
  logging::log<logging::level::info>("ok!");
  logging::log<logging::level::info>(
      "Max Connolly cells per auxiliary cell -> {}", max_t);
...
#else
  logging::log<logging::level::warn>("Connolly surface requires CGAL lib");
  return false;
#endif

  if (!flag) {
    logging::log<logging::level::err>("Connolly surface construction failed");
    return flag;
  }
  return flag;
}
...
bool ConnollySurface::preBoundaryProjection() {
  ...
  if (projBGP) {
    ...
    if (!flag) {
      logging::log<logging::level::err>("Cannot build 3D auxiliary grid");
      return flag;
    }
  }
  return true;
...
bool ConnollySurface::buildConnollyCGAL() {
  ...
  logging::log<logging::level::info>("Adjusting self intersection grid ");
  while (1) {
  ...
  for (unsigned int i = 0; i < ggrid; i++)
    for (unsigned int j = 0; j < ggrid; j++)
      for (unsigned int k = 0; k < ggrid; k++)
        // init the number of probes mapped in that place (the pointer is a
        // number)
  ...
  logging::log<logging::level::info>("Computing alpha shape complex....");
    ...
    if (ctype == Fixed_alpha_shape_3::REGULAR ||
      ...
      for (unsigned int i = 0; i < ll.size(); i++) {
        ...
        if (ctype == Fixed_alpha_shape_3::INTERIOR) {
          ...
          double ratio =
              (v1->point().weight() + dij2 - v2->point().weight()) / (2 * dij);
          ...
          // get by Pitagora theorem the distance from the torus center to the
          // probe
          double torus_center_probe_dist2 =
              v1->point().weight() - torus_center_v1_dist2;
  ...
  for (unsigned int i = 0; i < exposed.size(); i++)
    fprintf(fpExp, "C %f %f %f\n", delphi->atoms[exposed[i]]->pos[0],
            delphi->atoms[exposed[i]]->pos[1],
            delphi->atoms[exposed[i]]->pos[2]);
  fclose(fpExp);
  ...
  for (; ffit != alpha_shape.finite_facets_end(); ffit++) {
    ...
    if (ctype == Fixed_alpha_shape_3::REGULAR ||
      ...
      if (alpha_shape.classify(ffit->first) == Fixed_alpha_shape_3::INTERIOR) {
        ...
        const Vector3 nn =
            (facetp.second % 2 == 1)
                ? CGAL::normal(p1.point(), p2.point(), p3.point())
                : CGAL::normal(p1.point(), p3.point(), p2.point());
        n[0] = nn.x();
        n[1] = nn.y();
        n[2] = nn.z();
        ...
        const Vector3 nn =
            (facetp->second % 2 == 1)
                ? CGAL::normal<K>(p1.point(), p2.point(), p3.point())
                : CGAL::normal(p1.point(), p3.point(), p2.point());
        n[0] = nn.x();
        n[1] = nn.y();
        n[2] = nn.z();
      ...
      const double V =
          1. / 12. *
          sqrt(4 * D * E * F - F * X * X - D * Y * Y - E * Z * Z + X * Y * Z);
      ...
      // if it is singular this automatically clips the sphere without adding
      // planes
      if (!isRegular) {
        // tethraedron points
        // PP1 = P1;
      ...
      // If numSelfIntersections==0 and isSelfIntersecting=0 then this means
      // that the only self intersections is due to mirror facet
  ...
  for (; feit != alpha_shape.finite_edges_end(); feit++) {
    ...
    if (ctype == Fixed_alpha_shape_3::REGULAR ||
      ...
      double ratio2 =
          (v2->point().weight() + dij2 - v1->point().weight()) / (2 * dij);
      ...
      // get a random point on the COI (Circle Of Intersection). The COI can be
      ...
      <%lf,%lf,%lf>,%lf",(P.x()),(P.y()),(P.z()),0.1); of << buff2; of << "\n
      ...
      */
      ...
      if (ec->patch_type == REGULAR_EDGE_CELL) {
        ...
        if (fcv[0] == NULL || fcv[1] == NULL) {
          logging::log<logging::level::err>("Error at atoms {},{}", ec->id[0],
                                            ec->id[1]);
          logging::log<logging::level::err>(
        ...
        // if the clipping is in "AND" or "OR" mode. That is the angle is
        // respectively  <pi or >pi between planes

        // two border probes: this is the most common situation
        // recover the two reference planes and deduce if the "AND" or "OR"
        // configuaration is needed
        if (index == 2) {
          bool found = false;
          // get first plane
          ...
          ec->acute = orientation(fcv[0]->center, fcv[1]->center,
                                  ec->cutting_planes[0], ec->cutting_planes[1]);
        }
        ...
        if (index > 2) {
          ...
          // of the probes along the current edge sort the probe positions along
          // the COI in order to get a pair of planes every time.
          ...
          // very probably at least one facet is singular: do fast detection of
          // the first probe
      ...
      if ((bigR - probe_radius) >= 0)
        ...
        ec->self_intersection_radius =
            sqrt(-bigR * bigR + probe_radius * probe_radius);

        // if the edge is singular and self intersecting now probes pair must be
        // collected
      }
  ...
  ////////////////////////////////////////////////////////////////////////////////////////////////////////
  logging::log<logging::level::info>(
      "Checking {} probes for self intersections...", checkList.size());
...
void ConnollySurface::printSummary() {
  ...
  {
    logging::log<logging::level::info>("Number of ses cells -> {}",
                                       sesComplex.size());
    logging::log<logging::level::info>("Number of del_point cells -> {}",
                                       type[POINT_CELL]);
    logging::log<logging::level::info>("Number of regular del_edge cells -> {}",
                                       type[REGULAR_EDGE_CELL]);
    logging::log<logging::level::info>(
        "Number of singular del_edge cells -> {}", type[SINGULAR_EDGE_CELL]);
    logging::log<logging::level::info>(
        "Number of regular del_facet cells -> {}", type[REGULAR_FACE_CELL]);
    logging::log<logging::level::info>(
        "Number of singular del_facet cells -> {}", type[SINGULAR_FACE_CELL]);

    logging::log<logging::level::info>("cells {}", sesComplex.size());
    logging::log<logging::level::info>("del_point {}", type[POINT_CELL]);
    logging::log<logging::level::info>("rdel_edge {}", type[REGULAR_EDGE_CELL]);
    logging::log<logging::level::info>("sdel_edge {}",
                                       type[SINGULAR_EDGE_CELL]);
    logging::log<logging::level::info>("rdel_facet {}",
                                       type[REGULAR_FACE_CELL]);
    logging::log<logging::level::info>("sdel_facet {}",
                                       type[SINGULAR_FACE_CELL]);
  }
}
...
bool ConnollySurface::getProjection(double p[3], double* proj1, double* proj2,
  ...
  if (cells.size() == 0) {
    logging::log<logging::level::warn>("Empty cell in getProjection!");
    return false;
  }

  // Connolly surface is not that nice. Projections can't be analytical
  // everywhere. We are trying to project a point into a singularity? Old DelPhi
  // style.
  if (minDist == INFINITY) {
    bool fixed = false;
    ...
    for (it = cells.begin(); it != cells.end(); it++) {
      ...
      if (cc->patch_type == SINGULAR_EDGE_CELL ||
        ...
        if (ec->isSelfIntersecting) {
          // identify the nearest the common probes and check the feasibility of
...
#endif
              logging::log<logging::level::warn>("Singular edge projection");
            }
          }
...
#endif
        logging::log<logging::level::warn>(
            "Approximating bgp with grid point ");
      }
    } else {
      // non tangent continuity has been properly managed.
      // the obtained projection is the best thing allowed by Connolly
      // definition.
    }
  }
  return true;
...
void ConnollySurface::preProcessPanel() {
  ...
  gridConnollyCellMap2D = allocateVector<int>(last_rows_ind * last_cols_ind *
                                              MAX_CONNOLLY_CELLS_2D);
...
void ConnollySurface::getRayIntersection(
  ...
  if (panel == 0) {
    ...
    for (unsigned int iter = 0; iter < numcells; iter++) {
      ...
      bool ff = rayConnollyCellIntersection(pa, dir, sesComplex[it], &t1, &t2,
                                            &t3, &t4, ni, thdID);
      numInt++;
    ...
    for (unsigned int iter = 0; iter < numcells; iter++) {
      ...
      ff = rayConnollyCellIntersection(pa, dir, sesComplex[it], &t1, &t2, &t3,
                                       &t4, ni, thdID);
...
bool ConnollySurface::rayConnollyCellIntersection(double* orig, double* dir,
  ...
  if (patch_type == REGULAR_FACE_CELL || patch_type == SINGULAR_FACE_CELL) {
    ...
    logging::log<logging::level::err>(
        "Cannot get patch type in intersection test");
...
bool ConnollySurface::isFeasible(ConnollyCell* cc, double* point) {
  if (cc->patch_type == REGULAR_FACE_CELL ||
    ...
    for (unsigned int i = 0; i < fc->self_intersection_planes.size(); i++) {
      double tst = DOT(fc->self_intersection_planes[i], point) +
                   fc->self_intersection_planes[i][3];
      if (tst > 0)
        return false;
    }
    ...
    if (ec->additional_planes.size() != 0) {
      ...
      for (unsigned int i = 0; i < ec->flags.size(); i++, plane_index += 2) {
        ...
        if (acute) {
          double tst = DOT(ec->additional_planes[plane_index], point) +
                       ec->additional_planes[plane_index][3];
          if (tst > 0)
            continue;

          tst = DOT(ec->additional_planes[plane_index + 1], point) +
                ec->additional_planes[plane_index + 1][3];
          if (tst > 0)
            continue;
    ...
    // fast path
      ...
      if (ec->acute) {
        double tst =
            DOT(ec->cutting_planes[0], point) + ec->cutting_planes[0][3];
        if (tst > 0)
          return false;
  ...
  logging::log<logging::level::err>("Cannot get an answer in feasibility test");
  return false;
}
...
void ConnollySurface::projectToTorus(double* y, EdgeCell* ec, double* proj,
  ...
  projectToCircle(pp, ec->major_radius, torus_center, torus_plane, proj_major,
                  dist);
...
void ConnollySurface::saveConcaveSpherePatch(ofstream& of, FacetCell* fc,
  ...
  for (int l = 0; l < 4; l++) {
    snprintf(temp, sizeof(temp), "plane{<%f,%f,%f>,%f}", fc->planes[l][0],
             fc->planes[l][1], fc->planes[l][2],
             -(fc->planes[l][3]) / (sqrt((DOT(fc->planes[l], fc->planes[l])))));
    of << endl << temp;
  }
  ...
  if (fc->isSelfIntersecting) {
    ...
    for (unsigned int l = 0; l < fc->self_intersection_planes.size(); l++) {
      snprintf(temp, sizeof(temp), "plane{<%f,%f,%f>,%f}",
               ...
               -(fc->self_intersection_planes[l][3]) /
                   (sqrt((DOT(fc->self_intersection_planes[l],
                              fc->self_intersection_planes[l])))));
      of << endl << temp;
    }
  }
  ...
  snprintf(buff2, sizeof(buff2), "\n\n sphere { \n <%lf,%lf,%lf>,%lf",
           fc->center[0], fc->center[1], fc->center[2], probe_radius);
  of << buff2;
...
void ConnollySurface::saveAtomPatch(ofstream& of, PointCell* pc) {
  ...
  of << "\n// -------------- ATOM " << pc->id << "-------------------";
  snprintf(buff, sizeof(buff), "AtomClip%d", pc->id);
  // add
  of << "\n\n #declare " << buff << " =";
  of << "\n union {";
  double* sphere;
  ...
  snprintf(buff2, sizeof(buff2),
           "\n\n sphere { \n <%lf,%lf,%lf>,%lf  pigment{color Green}}",
           delphi->atoms[pc->id]->pos[0], delphi->atoms[pc->id]->pos[1],
           delphi->atoms[pc->id]->pos[2], delphi->atoms[pc->id]->radius);
  of << buff2;
  of << "\nobject{" << buff << "}}";
}
...
void ConnollySurface::saveEdgePatch(ofstream& of, EdgeCell* ec, int size,
  ...
  of << "\n\n";
  snprintf(clipplanes, sizeof(clipplanes), "Clipping_Planes%d", size);
  ...
  if (ec->patch_type != SINGULAR_EDGE_CELL) {
    ...
    if (ec->additional_planes.size() == 0) {
      ...
      for (int l = 0; l < 2; l++) {
        snprintf(
            buff3, sizeof(buff3), "\nplane{<%f,%f,%f>,%f}",
            ec->cutting_planes[l][0], ec->cutting_planes[l][1],
            ec->cutting_planes[l][2],
            -(ec->cutting_planes[l][3]) /
                (sqrt((DOT(ec->cutting_planes[l], ec->cutting_planes[l])))));
        of << buff3;
      }
      of << "\n}";
    } else {
      ...
      for (unsigned int i = 0; i < ec->additional_planes.size(); i += 2) {
        ...
        of << buff3;
        snprintf(buff3, sizeof(buff3), "\nplane{<%f,%f,%f>,%f}",
                 ec->additional_planes[i][0], ec->additional_planes[i][1],
                 ec->additional_planes[i][2],
                 -(ec->additional_planes[i][3]) /
                     (sqrt((DOT(ec->additional_planes[i],
                                ec->additional_planes[i])))));
        of << buff3;
        snprintf(buff3, sizeof(buff3), "\nplane{<%f,%f,%f>,%f}",
                 ec->additional_planes[i + 1][0],
                 ec->additional_planes[i + 1][1],
                 ec->additional_planes[i + 1][2],
                 -(ec->additional_planes[i + 1][3]) /
                     (sqrt((DOT(ec->additional_planes[i + 1],
                                ec->additional_planes[i + 1])))));
        of << buff3;
  ...
  snprintf(clip, sizeof(clip), "Clip%d", size);
  of << "\n\n #declare " << clip << " =";
  ...
  if ((bigR - probe_radius) < 0) {
    snprintf(
        ...
        (ec->clipping_center[0]), (ec->clipping_center[1]),
        (ec->clipping_center[2]), ec->clipping_radius, ec->center[0],
        ec->center[1], ec->center[2],
        sqrt(-bigR * bigR + probe_radius * probe_radius));
    of << buff2;
  } else {
    snprintf(
        ...
        //(clipping_center.x()),(clipping_center.y()),(clipping_center.z()),r_clipping);
        (ec->clipping_center[0]), (ec->clipping_center[1]),
        (ec->clipping_center[2]), ec->clipping_radius);
    of << buff2;
  }

  // global clipping object
  snprintf(buff, sizeof(buff), "Clipping_Object%d", (int)sesComplex.size());
  ...
  snprintf(buff2, sizeof(buff2),
           "\n matrix <%lf,%lf,%lf,\n%lf,%lf,%lf,\n%lf,%lf,%lf,\n0,0,0>", u[0],
           u[1], u[2], w[0], w[1], w[2], v[0], v[1], v[2]);
  of << buff2;
  // of << "\n translate"<< "<" << torus_center.x() << "," << torus_center.y()
  // << "," << torus_center.z() << ">";
  of << "\n translate"
     << "<" << ec->center[0] << "," << ec->center[1] << "," << ec->center[2]
     << ">";
  of << "\nsturm";
  of << "\n bounded_by { " << buff << " } \n clipped_by{ bounded_by}}";
...
void ConnollySurface::getCoi(double* torus_center, double rcoi,
  ...
  for (int i = 0; i < numPoints; i++) {
    sampledPoints[i][0] =
        torus_center[0] + rcoi * (cos(tcoi) * u[0] + sin(tcoi) * v[0]);
    sampledPoints[i][1] =
        torus_center[1] + rcoi * (cos(tcoi) * u[1] + sin(tcoi) * v[1]);
    sampledPoints[i][2] =
        torus_center[2] + rcoi * (cos(tcoi) * u[2] + sin(tcoi) * v[2]);
    tcoi += step;
