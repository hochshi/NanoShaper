...
const T& Octree<T, AS>::at(int x, int y, int z) const {
  ...
  while (size != aggregateSize_) {
    if (!n) {
      ...
      size /= 2;
      n = reinterpret_cast<Branch*>(n)->child(!!(x & size), !!(y & size),
                                              !!(z & size));
    } else {
...
void Octree<T, AS>::eraseRecursive(Node** node, int size, int x, int y, int z) {
  ...
  if (size != aggregateSize_) {
    if ((*node)->type() == BranchNode) {
      ...
      eraseRecursive(&b->child(!!(x & size), !!(y & size), !!(z & size)), size,
                     x, y, z);
      ...
      size /= 2;
      int childIndex =
          (x & size ? 1 : 0) | (y & size ? 2 : 0) | (z & size ? 4 : 0);
...
template <typename T, int AS>
Octree<T, AS>::Node::Node(NodeType type) : type_(type) {}
...
template <typename T, int AS>
Octree<T, AS>::Branch::Branch() : Node(BranchNode) {
  memset(children, 0, sizeof(children));
}

template <typename T, int AS>
Octree<T, AS>::Branch::Branch(const Branch& b) : Node(BranchNode) {
  for (int i = 0; i < 8; ++i) {
    if (b.child(i)) {
      switch (b.child(i)->type()) {
        ...
        case AggregateNode:
          child(i) =
              new Aggregate(*reinterpret_cast<const Aggregate*>(b.child(i)));
          break;
      }
    } else {
      child(i) = 0;
...
template <typename T, int AS>
const typename Octree<T, AS>::Node* Octree<T, AS>::Branch::child(int x, int y,
                                                                 int z) const {
  assert(x == 0 || x == 1);
  assert(y == 0 || y == 1);
  assert(z == 0 || z == 1);
...
template <typename T, int AS>
typename Octree<T, AS>::Node*& Octree<T, AS>::Branch::child(int x, int y,
                                                            int z) {
  assert(x == 0 || x == 1);
  assert(y == 0 || y == 1);
  assert(z == 0 || z == 1);
...
template <typename T, int AS>
Octree<T, AS>::Aggregate::Aggregate(const T& v) : Node(AggregateNode) {
  for (int i = 0; i < AS; ++i) {
    for (int j = 0; j < AS; ++j) {
      for (int k = 0; k < AS; ++k) {
        value_[i][j][k] = v;
...
template <typename T, int AS>
Octree<T, AS>::Leaf::Leaf(const T& v) : Node(LeafNode), value_(v) {}
...
void Octree<T, AS>::zSliceRecursive(_Array2D<T> slice, const Node* node,
  ...
  if (!node) {
    ...
    for (int i = 0; i < 2; ++i) {
      for (int j = 0; j < 2; ++j) {
        zSliceRecursive(
            slice.subarray(i * size, j * size, (i + 1) * size, (j + 1) * size),
            reinterpret_cast<const Branch*>(node)->child(j, i,
                                                         !!(targetZ & size)),
            size, x, y, z, targetZ);
      }
    }
