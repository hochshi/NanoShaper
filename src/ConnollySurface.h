...
/** @brief This class builds and converts to a DelPhi suitable representation
the Connolly-Richards Surface. All the gathered info is analytically computed
both the intersections and the projections. First the alpha shape of the set of
atoms is computed, then from that the Connolly surface is computed by build all
the patches.
...
class ConnollySurface : public Surface {
 ...
 private:
  ...
  // typedef CGAL::Polyhedron_3<K> Polyhedron; typedef
  // Polyhedron::Halfedge_around_facet_circulator        HF_circulator; typedef
  // CGAL::Vector_3<K>
  // Vector3; typedef CGAL::Plane_3<K>
  // Plane3; typedef CGAL::Ray_3<K>
  // Ray3; typedef CGAL::Line_3<K>
  // Line3; typedef
  // CGAL::Triangulation_vertex_base_with_info_3<PointCell*,Traits>
  // Vertex_with_info; typedef
  // CGAL::Fixed_alpha_shape_vertex_base_3<Traits,Vertex_with_info>
  // Vb; typedef CGAL::Triangulation_cell_base_with_info_3<
  // FacetCell*[4],Traits> Cell_with_info; typedef
  // CGAL::Regular_triangulation_cell_base_3<Traits,Cell_with_info>
  // RT_Cell_with_info; typedef
  // CGAL::Fixed_alpha_shape_cell_base_3<Traits,RT_Cell_with_info>         Fb;
  // typedef CGAL::Triangulation_data_structure_3<Vb,Fb>
  // Tds; typedef Tds::Cell_circulator
  // Cell_circulator; typedef CGAL::Regular_triangulation_3<Traits,Tds>
  // Rt; typedef CGAL::Fixed_alpha_shape_3<Rt>
  // Fixed_alpha_shape_3;
  ...
  //   typedef CGAL::Triangulation_vertex_base_with_info_3<int, K, Vb0> Vb1;
  //   typedef CGAL::Fixed_alpha_shape_vertex_base_3<K>          Vb0;
  // typedef CGAL::Triangulation_vertex_base_with_info_3<PointCell*, K, Vb0> Vb;
  typedef CGAL::Fixed_alpha_shape_vertex_base_3<Vb0::Geom_traits, Vb0> Vb1;
  ...
  typedef Rt::Triangulation_data_structure::Vertex_iterator Vertex_iterator;
 ...
 public:
  ...
  double getProbeRadius() { return probe_radius; }

  void setSavePovRay(bool ss) { savePovRay = ss; }

  bool getSavePovRay() { return savePovRay; }

  /**for the 3d grid set the max grid size and the maximal number of patches
   * inside a grid cube*/
  ...
  /**for the 2d grid set the max grid size and the maximal number of patches
  inside a grid cube. The grid cube itself does not exist just a reference,
  indeed the real quantity is MAX_CONNOLLY_CELLS_2D that is the number of
  patches along the grid tube*/
  ...
  int getMaxProbes() { return MAX_PROBES; }

  void setSIPerfil(double si) { si_perfil = si; }

  double getSIPerfil() { return si_perfil; }
