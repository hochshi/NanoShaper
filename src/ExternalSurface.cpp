...
void ExternalSurface::init(void)
{
...
void ExternalSurface::clear(void)
{
	;
}
...
int ExternalSurface::getNumPatches(void)
{
...
bool ExternalSurface::isPatchBasedRayTracingSupported (void)
{
	return false;
}
...
void ExternalSurface::printSummary(void)
{
	return;
}
...
double ExternalSurface::getSurfaceArea(void)
{
	cout << endl << WARN << "Cannot compute area in an external surface!";
	return 0.0;
}
...
double ExternalSurface::getVolume(void)
{
	int NX = delphi->nx;
	int NY = delphi->ny;
	int NZ = delphi->nz;
	...
	totalVolume += delphi->side*delphi->side*delphi->side * (count / 6.);

	return totalVolume;
}


bool ExternalSurface::getSurf(double *surf_volume, bool optimize_grids, bool fillCav, double cav_vol)
{
	...
	*surf_volume = 0;
	...
	{
		int check = fscanf(fproj, "%d %d %d %lf %lf %lf %lf %lf %lf",
						   &(ind[0]),&(ind[1]),&(ind[2]),
						   &(bgpdata[0]),&(bgpdata[1]),&(bgpdata[2]),
	...
	for (iz=0; iz<NZ; iz++)
		for (iy=0; iy<NY; iy++)
			for (ix=0; ix<NX; ix++)
			{
				...
				if (!optimizeGrids)
				{
					// delphi->status[iz][iy][ix] = ((char)tempint);
					// delphi->STATUSMAP(ix,iy,iz,NX,NY) = tempint;
					write3DVector<int>(delphi->status,tempint,ix,iy,iz,NX,NY,NZ);
				}
				else
				{
					writeBilevelGrid<int>(delphi->bilevel_status,STATUS_POINT_TEMPORARY_OUT,tempint,ix,iy,iz,NX,NY,NZ);
				}
			}
	...
	if (ix != NX)
	{
		if (!optimizeGrids)
			...
			cout << endl << WARN << "Error reading bilevel status map!";
...
inline void ExternalSurface::getRayIntersection(double p1[3],double p2[3], vector<pair<VERTEX_TYPE,VERTEX_TYPE*>> &intersections,bool computeNormals,int thread_id)
{
	intersections.clear();
...
inline void ExternalSurface::printRayIntersection(double pa[3], double pb[3])
{
	;
}
#endif
...
bool ExternalSurface::getProjection(double p[3],double *proj1,double *proj2, double *proj3,
