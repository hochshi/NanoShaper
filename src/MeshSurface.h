...
#define GRID_TRIANGLEMAP_2D(i, j, k, NA, NB) \
  gridTriangleMap2D[(k) + (MAX_TRIANGLES_2D - 1) * ((j) + (NB) * (i))]
#define GRIDTRIANGLEMAP(i, j, k, l, NX, NY, NZ) \
  gridTriangleMap[(l) + (MAX_TRIANGLES - 1) * ((k) + (NZ) * ((j) + (NY) * (i)))]
...
class MeshSurface : public Surface {
 protected:
  ...
  bool point2triangle(double P[3], double A[3], double B[3], double C[3],
                      double w[4], double* proj, double* dist, double* normal,
                      int planeID, int va, int vb, int vc);
